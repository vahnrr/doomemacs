...
** Error when opening/saving files on Alpine
Since Alpine is a musl-based distribution, by default, =/lib/ld-linux.so.2=
doesn't exist. [[https://wiki.alpinelinux.org/wiki/Running_glibc_programs#gcompat][Its wiki offers a fix]] which can be summed up to:

#+begin_src shell
# Install the compatibility layer which creates '/lib/ld-linux.so.2'.
apk add gcompat

# From now on, start Emacs and make it use this library instead of musl.
LD_PRELOAD=/lib/ld-linux.so.2 emacs
#+end_src
